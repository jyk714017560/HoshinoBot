...
sv = Service('pcr_calculate', enable_on_default=True, help_='''合刀计算器''')
...
async def cal(bot, ev: CQEvent):
    ...
    if eigen == "10":  # cal 1500
        ...
        outp += "刀数 / 满补所需伤害\n"
        for i in range(1, 7):
            outp += f"{i}刀 \t {int(math.ceil((delta + boss/(i-1+21/90))/10000))}w\n"
        ...
        if x >= boss:
            ...
            if y < 90:
                ...
                if z > 10000:
                    outp += f"\n垫入{math.ceil(z/10000)}w伤害可满补"
                else:
                    outp += f"\n垫入{z}伤害可满补"
            await bot.finish(ev, outp)
        y = int(math.ceil((delta + 90 / 21 * (boss - x))/10000))
        yy = int(math.ceil((delta + boss - 21 / 90 * x)/10000))
        outp += f"另一刀伤害达到{yy}w(先出)/{y}w(后出)即可满补。"
        await bot.finish(ev, outp)
        ...
        if y > 10000:
            outp += f"还需垫入{math.ceil(y/10000)}w伤害"
        elif y > 0:
            outp += f"还需垫入{y}伤害"
        else:
